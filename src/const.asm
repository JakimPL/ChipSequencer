; Envelope
    %define ENVELOPE_SIZE 12
    %define ENVELOPE_BASE_VOLUME 0
    %define ENVELOPE_SUSTAIN_LEVEL 2
    %define ENVELOPE_ATTACK 4

; Sequence
    %define SEQUENCE_NOTES 1

; Channel
    %define CHANNEL_SIZE 6
    %define CHANNEL_ENVELOPE_INDEX 0
    %define CHANNEL_ENVELOPE_MODE 1
    %define CHANNEL_SEQUENCE_INDEX 2
    %define CHANNEL_SEQUENCE_CURRENT_NOTE 3
    %define CHANNEL_OSCILLATOR_INDEX 4
    %define CHANNEL_TRANSPOSE 5
